...
;;; Some symbols are imported from the underlying Lisp into GL.  The following
;;; circustances require importing:

;;;   translatable symbols generated by the readtable (e.g. quote, lambda),
;;;   self-evaluating symbols (not having T or NIL makes my brain hurt), and
;;;   lambda-list keywords (so I can easily expand gl:defun), *features*, and
;;;   Lisp type names.
...
;;; must be imported from Lisp into GL.  Note that the symbols generated by
;;; backquote need not be imported, since we have implemented our own version of
;;; backquote in glt/lisp/backquote.lisp.

(defmacro underlying-lisp-symbols-exported-by-gl ()
...
(import
  (underlying-lisp-symbols-exported-by-gl)
  (find-package "GL"))
...
;;; All symbols available in GLT defined packages are exported from within this
...
;;; naming the symbols not inherited from Lisp that are to be exported from GL.

(defmacro exported-gl-print-names ()
  ''("SPECIAL-FORM-P"
     "MACRO-FUNCTION"
     "DEFMACRO"
     "MACROEXPAND"
     ...
     ;; GL specific extentions for code walking, for performing full
     ...
     ;; Foreign functions
     "DEF-GL-FOREIGN-FUNCTION"

     ;; The following symbols are GL extensions to the declarations in Common
     ;; Lisp.
     "MACROS-ONLY"
     "RETURN-TYPE"
     ...
     "DEFINE-DECLARATION"
     "PARSE-MACRO"
     ;; "ENCLOSE", skipped, no lexical closures in GL.  -jra 4/2/95
...
;;; The following call actually interns the print names into GL and then exports
;;; those symbols.

(export (intern-in-package
	  "GL"
	  (exported-gl-print-names))
	(find-package "GL"))
