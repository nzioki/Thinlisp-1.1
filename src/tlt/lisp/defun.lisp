...
;;; This module implements the macros `gl:defun' and `gl:def-c-translation'.  These
...
(defun choose-variable-type-given-walked-env (variable inner-env)
  (multiple-value-bind (type? local? decls)
      (gl:variable-information variable inner-env)
...
(defun collect-computed-ftypes (form env inner-env required-type)
  ...
  (cond
    ((and (consp form)
     ...
     (gl:destructuring-bind-strict (nil name lambda-list . doc-decls-body) form
      (push
	`(gl:declaim
	       (,(if (gl:declaration-information 'gl:return-type inner-env)
		     'ftype
		     'computed-ftype)
		 (function
		   ,(loop for lambda-elt in lambda-list
			  ...
			  (if (eq lambda-elt '&optional)
				...
				(choose-variable-type-given-walked-env
				  arg-var inner-env))))
		   ,(or (gl:declaration-information 'gl:return-type inner-env)
...
;;; The macro `gl:defun' defines functions.  It differs from CL defun in that
;;; some of the argument list features of CL are not available in GL.  In
;;; particular, GL does not directly support &key or &rest arguments in
;;; functions, but uses macros to provide those facilities.

;;; If a "function" is defined with &key or &rest args, then actually gl:defun
...
;;; When not translating, `gl:defun' expands into a declaim of the
...
;;; When not translating, we are either in an explicit call to gl:walk for some
...
(def-gl-macro gl:defun (name lambda-list &body doc-decls-body)
  ...
  (if (or (memq '&key lambda-list) (memq '&rest lambda-list))
	...
	(multiple-value-bind (decls body)
	  ...
	  `(gl:progn
	     (gl:defmacro ,name ,macro-lambda-list
	       ,(if rest-arg?
		    `(list ',func-name
		 ...
		 `((gl:declaim (inline ,func-name))))
	     (gl:defun ,func-name ,func-lambda-list
	       ,@decls
	       (gl:block ,name
		 ,@body)))))
	   ...
	   ,@(when (and (eq (function-decl name 'inline) 'inline)
		 ...
		 (let ((var-list (loop for arg in lambda-list
					 ...
					 collect (if (consp arg)
						     (car arg)
						     arg))))
		   `((gl:define-compiler-macro ,name ,lambda-list
		       ,(cond
			  ...
			  ((and (or (null decls)
				    (and (null (cdr decls))
					 (null (cddar decls))
					 (eq (caadar decls) 'gl:return-type)))
				(= (length body) 1)
			   ...
			   (let ((declared-result-type?
				   (loop named type-search
				     ...
				     (loop for decl in (cdr declare) do
				       (when (eq (car decl) 'gl:return-type)
					 (return-from type-search (second decl)))))))
			     ``(gl:let ,(list ,@(loop for var in var-list
						      collect `(list ',var ,var)))
				 ,@',decls
				 (,@',(if declared-result-type?
					  `(gl:the ,declared-result-type?)
					  '(gl:progn))
				    (gl:block ,',name
				      ,@',body)))))))))))
	,walked-named-lambda))))
...
(defmacro def-c-translation (name function-arglist ftype-and-macro-decls
  ...
  (gl:destructuring-bind-strict (&key ftype)
    ...
    (let* ((macro (cons-cdr ftype-and-macro-decls))
	   ...
	   (home-system (or *current-system-name* 'gl-user::gl))
	   (home-module (or *current-module-name* 'gl-user::glt))
	   (c-arg-types
	     (loop for lisp-type in (cons-car ftype)
		   collect (c-type-for-lisp-type lisp-type))))
			      
      `(progn
	 (gl:declaim (ftype ,(cons 'function ftype) ,name)
		     (c-translator
